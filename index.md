...
Another way to understand to which degree the goodness of fit increases when we add the clusters is to plot the error made by each model. One common of such error plots is the prediction/ground truth scatter plot. The better the model, the closer to a straitline the plot will be.
